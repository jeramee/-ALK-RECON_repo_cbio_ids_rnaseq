...
ALK-RECON organizes one core idea:

> Don’t treat ALK resistance as “one mutation = one answer.”  
...
1. **On-target ALK changes**  
   Binding/conformation shifts (e.g., solvent-front **G1202R**, gatekeeper **L1196M**, **compound ALK mutations**)

2. **Off-target bypass**  
   Alternate drivers restore survival signaling (common examples: **MET**, **EGFR**, **MAPK** pathway nodes)

3. **Tolerance / persistence**  
   Drug-tolerant survivors without stable resistance mutations (often described with **HDAC/DNMT/BET-type** state logic)
...
Every dossier aims to answer:
- What did we see (**KNOWN**)?
- What did we infer?
- What’s speculative?
...
Optional docs you can add under `docs/`:
- `01_GLOSSARY.md`, `02_MECHANISM_MAP.md`, `03_STRATEGY_OPTIONS.md`
- `04_DECISION_TREE.md`, `05_VALIDATION_PLAN.md`, `07_EXEC_SUMMARY_1PAGE.md`
...
python -m alk_recon.cli \
  --input examples/variants_sample.tsv \
  --out out
```
...
A typical `out/dossiers/<case_id>.md`:
...
Rule-first is intentional (labels are scarce/noisy; interpretability matters).
...
If you want this reusable across cohorts, keep config in YAML (e.g., `config/alk_recon.yaml`) and keep the CLI clean.
...
LLMs can improve readability, but must:
- cite evidence IDs
- never invent facts
- label uncertainty
...
- ALK compound mutation (≥2 ALK resistance mutations)
...
Meaningful evidence usually requires:
- engagement → signaling shutdown → phenotype
- mutant coverage (single + compound)
- durability assays (days–weeks), not just short-term viability
...
- **RNA-seq:** count columns must match metadata `sample_id`.
- **Timepoints:** keep `timepoint_id` consistent (`baseline`, `progression_1`, etc.).
